...
impl Exporter for CsvExporter {
    fn export<I, W>(matches: I, out: W) -> Result<(), AnnisExportError>
    where
        I: IntoIterator<Item = Result<Match, GraphAnnisError>>,
        W: Write,
    {
        let mut csv_writer = csv::Writer::from_writer(out);
        let mut match_token_count = None;

        for (i, m) in matches.into_iter().enumerate() {
            let m = m?;

            match (match_token_count, m.match_token_count()) {
                (None, n) => {
                    match_token_count = Some(n);
                    csv_writer.write_record(iter::once("Number".into()).chain((0..=2 * n).map(
                        |k| match (n, k % 2, k / 2) {
                            (0, 0, 0) => "Context".into(),
                ...
                (Some(count), n) if n != count => {
                    return Err(AnnisExportError::DifferentMatchTokenCount(count, n))
                }
                _ => (),
            }

            csv_writer.write_record(iter::once((i + 1).to_string()).chain(TokenColumns::new(m)))?;
        }
...
mod tests {
    ...
    macro_rules! csv_exporter_test {
        ($($name:ident: [$([$($part:tt)*])*] => $expected:expr)*) => {
            $(
                #[test]
                fn $name() {
    ...
    csv_exporter_test! {
        ...
        no_match_token_without_context: [
            []
        ] => "
            Number,Context
            1,
        ...
        one_match_token_without_context: [
            [(M "abc")]
        ] => "
        ...
        two_match_tokens_without_context: [
            [(M "abc") (M "def")]
        ] => "
        ...
        many_match_tokens_without_context: [
            [(M "abc") (M "def") (M "ghi")]
        ] => "
        ...
        many_match_tokens_with_context: [
