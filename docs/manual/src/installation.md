...
> MacOS is currently not supported. [Let us know](feedback.md) if you are interested in MacOS support.
...
> **Note:** We strongly recommend installing updates, so that you keep up with new features and bugfixes.
...
The Annimate AppImage is a self-contained application bundle that contains all of its dependencies pre-bundled and runs on all common Linux distributions such as Ubuntu, Debian, openSUSE, RHEL, CentOS, Fedora etc. without requiring a dedicated installation step. It can update itself automatically whenever a new version of Annimate becomes available.
...
2. Make it executable:
   ...
   ```
3. Afterwards, you can run Annimate just by running the `.AppImage` file:
   ```shell
    ./Annimate*.AppImage
   ```
...
Since there is no dedicated installation step, updating in this case just means that the `.AppImage` file is replaced with a newly downloaded one. Note that the name of the `.AppImage` file (which includes a version number) stays the same, even though it now contains a newer version. If you want to avoid this, consider giving the file a version-independent name.
