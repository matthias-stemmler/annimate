...
impl FromStr for ProjectFile {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let project_file: ProjectFile = toml::from_str(s)?;

        if project_file.format_version == 0 || project_file.format_version > FORMAT_VERSION {
            Err(AnnimateReadFileError::UnsupportedVersion {
                version: project_file.format_version,
            })
        } else {
            Ok(project_file)
        }
    ...
    P: AsRef<Path>,
{
    ...
    let project_file: ProjectFile =
        fs::read_to_string(path)?
            .parse()
            .map_err(|err| AnnimateError::FailedToReadProject {
                path: path.into(),
                err,
            })?;
...
fn to_string_pretty(project_file: ProjectFile) -> String {
    ...
    document["format-version"] = i64::from(project_file.format_version).into();
